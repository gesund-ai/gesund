...
from .miseval_metrics import MisEval
...
class PlotMisEval:
    def __init__(
        ...
        if ground_truth_dict:
            ...
            self.miseval_metrics = MisEval(class_mappings)
            self.class_mappings = class_mappings
            self.validation_utils = ValidationUtils(meta)

    def highlighted_overall_metrics(self):
        metrics_dict = self.miseval_metrics.calculate_highlighted_overall_metrics(
    ...
    def study_classbased_table(self, target_attribute_dict):
        ...
        if target_attribute_dict:
            ...
            metrics_dict = self.miseval_metrics.calculate_statistics_classbased_table(
    ...
    def metrics_by_meta_data(self, target_attribute_dict=None):
        ...
        try:
            ...
            miseval_metrics = MisEval(self.class_mappings)
            artifacts = self.miseval_metrics.create_artifacts(
    ...
    def main_metric(self):
        metrics_dict = (
            metrics_dict
        ) = self.miseval_metrics.calculate_highlighted_overall_metrics(
            gt=self.ground_truth_dict, pred=self.prediction_dict
        )
        payload_dict = {
            "type": "main_metric",
    ...
    def blind_spot_metrics(self, target_attribute_dict, threshold):
        ...
        class_metrics = self.miseval_metrics.calculate_statistics_classbased_table(
            gt=filtered_gt, pred=filtered_pred
        )

        overall_metrics = self.miseval_metrics.calculate_highlighted_overall_metrics(
            gt=filtered_gt, pred=filtered_pred
        )
    ...
    def create_artifacts(self, artifacts_path):
        status = False
        try:
            self.miseval_metrics.create_artifacts(
