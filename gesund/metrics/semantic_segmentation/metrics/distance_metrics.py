...
def _assert_is_numpy_array(name, array):
    ...
    :param name: A string representing the name of the variable for error messages.
    :param array: The variable to be checked.
    :raises ValueError: If `array` is not a numpy array.
    """
...
def _check_nd_numpy_array(name, array, num_dims):
    ...
    :param name: A string representing the name of the variable for error messages.
    :param array: The variable to be checked.
    :param num_dims: An integer representing the expected number of dimensions.
    :raises ValueError: If `array` does not have the expected number of dimensions.
    """
...
def _check_2d_numpy_array(name, array):
    ...
    :param name: A string representing the name of the variable for error messages.
    :param array: The variable to be checked.
    :raises ValueError: If `array` is not a 2D array.
    """
...
def _check_3d_numpy_array(name, array):
    ...
    :param name: A string representing the name of the variable for error messages.
    :param array: The variable to be checked.
    :raises ValueError: If `array` is not a 3D array.
    """
    _check_nd_numpy_array(name, array, num_dims=3)


def _assert_is_bool_numpy_array(name, array):
    """Raises an exception if `array` is not a boolean numpy array.

    :param name: A string representing the name of the variable for error messages.
    :param array: The variable to be checked.
    :raises ValueError: If `array` is not a boolean numpy array.
    """
...
def _compute_bounding_box(mask):
    ...
    This function generalizes to arbitrary number of dimensions greater than or equal to 1.

    :param mask: The 2D or 3D numpy mask, where '0' means background and non-zero means foreground.
...
def _sort_distances_surfels(distances, surfel_areas):
    """Sorts the two lists with respect to the tuple of (distance, surfel_area).

    :param distances: The distances from A to B (e.g. `distances_gt_to_pred`).
    :param surfel_areas: The surfel areas for A (e.g. `surfel_areas_gt`).
    :return: A tuple of the sorted (distances, surfel_areas).
    """
...
def compute_surface_distances(mask_gt, mask_pred, spacing_mm):
    ...
    and `spacing_mm` must be a 3-element list.
    ...
    :param mask_gt: 2-dim (resp. 3-dim) bool Numpy array. The ground truth mask.
    :param mask_pred: 2-dim (resp. 3-dim) bool Numpy array. The predicted mask.
    :param spacing_mm: 2-element (resp. 3-element) list-like structure. Voxel spacing
      in x0 and x1 (resp. x0, x1, and x2) directions.
    :return: A dict with:
        ...
        - "surfel_areas_gt": 1-dim numpy array of type float. The length of the
          ground truth contours in mm (resp. the surface elements area in
          mm^2) in the same order as distances_gt_to_pred.
        - "surfel_areas_pred": 1-dim numpy array of type float. The length of the
          predicted contours in mm (resp. the surface elements area in
          mm^2) in the same order as distances_gt_to_pred.
    :raises ValueError: If `mask_gt` and `mask_pred` are not the same dimensionality.
    """
...
def compute_average_surface_distance(surface_distances):
    ...
    :param surface_distances: A dictionary containing:
        - "distances_gt_to_pred": Array of distances from ground truth to predicted surface.
        - "distances_pred_to_gt": Array of distances from predicted to ground truth surface.
        - "surfel_areas_gt": Array of surface areas for ground truth.
        - "surfel_areas_pred": Array of surface areas for predicted.

    :return: A tuple containing:
        - Average distance (float): The average distance (in mm) from the ground truth surface to the predicted surface.
        - Average distance (float): The average distance from the predicted surface to the ground truth surface.
    """
...
def compute_robust_hausdorff(surface_distances, percent):
    ...
    Computes the robust Hausdorff distance, which uses the `percent` percentile 
    of the distances instead of the maximum distance. The percentage is computed 
    by correctly taking the area of each surface element into account.
    ...
    :param surface_distances: A dictionary containing:
        ...
        - "surfel_areas_gt": Array of surface areas for ground truth.
        - "surfel_areas_pred": Array of surface areas for predicted.
    :param percent: A float value between 0 and 100 representing the percentile to compute.

    :return: A float value representing the robust Hausdorff distance in mm.
    """
    distances_gt_to_pred = surface_distances["distances_gt_to_pred"]
    distances_pred_to_gt = surface_distances["distances_pred_to_gt"]
    surfel_areas_gt = surface_distances["surfel_areas_gt"]
...
def compute_surface_overlap_at_tolerance(surface_distances, tolerance_mm):
    ...
    :param surface_distances: A dictionary containing:
        ...
        - "surfel_areas_gt": Array of surface areas for ground truth.
        - "surfel_areas_pred": Array of surface areas for predicted.
    :param tolerance_mm: A float value representing the tolerance in mm.

    :return: A tuple containing:
        - Overlap fraction (float): The overlap fraction in [0.0, 1.0] of the ground truth surface with the predicted surface.
        - Overlap fraction (float): The overlap fraction in [0.0, 1.0] of the predicted surface with the ground truth surface.
    """
    distances_gt_to_pred = surface_distances["distances_gt_to_pred"]
    distances_pred_to_gt = surface_distances["distances_pred_to_gt"]
    surfel_areas_gt = surface_distances["surfel_areas_gt"]
...
def compute_surface_dice_at_tolerance(surface_distances, tolerance_mm):
    ...
    Computes the surface DICE coefficient at a specified tolerance. Not to be 
    confused with the standard volumetric DICE coefficient. The surface DICE 
    ...
    the other surface is less than or equal to the specified tolerance. 
    ...
    :return: A float value representing the surface DICE coefficient in [0.0, 1.0].
    """
    distances_gt_to_pred = surface_distances["distances_gt_to_pred"]
    distances_pred_to_gt = surface_distances["distances_pred_to_gt"]
    surfel_areas_gt = surface_distances["surfel_areas_gt"]
...
def compute_dice_coefficient(mask_gt, mask_pred):
    ...
    Computes the Sørensen-Dice coefficient between the ground truth mask `mask_gt`
    and the predicted mask `mask_pred`.

    :param mask_gt: A 3D numpy array of type bool representing the ground truth mask.
    :param mask_pred: A 3D numpy array of type bool representing the predicted mask.

    :return: A float value representing the Dice coefficient. If both masks are empty, the result is NaN.
    """
